...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.error('FLORA Platform React Boundary Caught Error:', error, errorInfo);
  }
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white p-6">
          <div className="max-w-md w-full bg-slate-800 border border-slate-700 rounded-xl p-6 text-center shadow-xl">
            ...
            <p className="text-slate-400 text-sm mb-6">
              {this.state.error?.message || 'An unexpected rendering error occurred. Please refresh the component view.'}
            </p>
            <button
